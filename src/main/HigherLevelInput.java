...
public class HigherLevelInput {
	...
	public void updateAlways(){
		...
		if(s.contains("w"))
			camera.move(0, Settings.moveSpeedWASD*GameController.timePassed);
		if(s.contains("s"))
			camera.move(0, -Settings.moveSpeedWASD*GameController.timePassed);
		if(s.contains("d"))
			camera.move(Settings.moveSpeedWASD*GameController.timePassed, 0);
		if(s.contains("a"))
			camera.move(-Settings.moveSpeedWASD*GameController.timePassed, 0);
		if(s.contains("y"))
			camera.moveRot(0, 0, -Settings.moveSpeedWASD*GameController.timePassed);
		if(s.contains("x"))
			camera.moveRot(0, 0, Settings.moveSpeedWASD*GameController.timePassed);
		...
		if(camera.mouseX<Settings.moveMouseOnEdgeThreshhold){
			...
			camera.move(-Settings.moveSpeedWASD*(1f-q)*GameController.timePassed, 0);
		}
		...
		if(camera.mouseY<Settings.moveMouseOnEdgeThreshhold){
			...
			camera.move(0, Settings.moveSpeedWASD*(1f-q)*GameController.timePassed);
		}
