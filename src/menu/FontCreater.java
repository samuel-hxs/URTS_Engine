...
public class FontCreater {
	...
	public static void create(String name, Font font, int xPos, int yPos, int rows, boolean blacked) throws Exception{
			...
			if(blacked){
				...
				for (int k1 = -1; k1 < 2; k1++) {
					for (int k2 = -1; k2 < 2; k2++) {
						g.drawString(""+charSet.charAt(i), coX*maxGlyphSizeX+xPos+1+k1, k2+coY*maxGlyphSizeY+yPos-4);
					}
				}
				g.setColor(Color.white);
			}
			g.drawString(""+charSet.charAt(i), coX*maxGlyphSizeX+xPos+1, coY*maxGlyphSizeY+yPos-4);
			
			wr.println("Char:"+charSet.charAt(i)+" "+(coX*maxGlyphSizeX+xPos)+" "+
					((coY-1)*maxGlyphSizeY+yPos)+" "+(maxGlyphSizeX-1)+" "+(maxGlyphSizeY-1));
		...
		File outputfile = new File(dir+"tex.png");
